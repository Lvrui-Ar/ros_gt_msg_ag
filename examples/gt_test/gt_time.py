...
def move(frequency, direction, pub, control, speed):
    """
    根据给定的方向和时间移动机器人。
    ...
    # rospy.loginfo(f"平均每次指令运动的时间是：{float(average)}")

if __name__ == "__main__":
    try:
        rospy.init_node("pub_command1")
        pub = rospy.Publisher("/gt_time", gt_control, queue_size=100)
    ...
    time=end - start
    average=time/10
    rospy.loginfo(f"平均每次指令运动的时间是：average:{average}")
    # move(10, "left", pub, controls["left_1"], controls["left_1"].x)
    # move(5, "backward", pub, controls["backward_1"], controls["backward_1"].x)
